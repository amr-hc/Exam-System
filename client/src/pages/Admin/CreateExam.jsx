...
const CreateExam = () => {
  ...
  const handleCorrectAnswerChange = (qIndex, aIndex) => {
    const newQuestions = [...exam.questions];
    newQuestions[qIndex].answers = newQuestions[qIndex].answers.map(
      (answer, index) => ({
        ...answer,
  ...
  const handleSubmit = (e) => {
    ...
    const formattedExam = {
      ...exam,
      duration: `${Math.floor(exam.duration / 60)}:${(
        "0" +
        (exam.duration % 60)
      ).slice(-2)}`,
    };
        ...
        <div key={qIndex}>
          ...
          <Form.Group as={Row} controlId={`formDegree${qIndex}`}>
            ...
            <Col sm={10}>
              <Form.Control
                ...
                onChange={(e) => handleDegreeChange(qIndex, e)}
              />
            </Col>
          </Form.Group>
          {question.answers.map((answer, aIndex) => (
            <Form.Group
              as={Row}
              key={aIndex}
              controlId={`formAnswer${qIndex}-${aIndex}`}
            >
              <Form.Label column sm={2}>
                Answer {aIndex + 1}
              </Form.Label>
